...
class AdapterEvaluator:
    ...
    def load_adapter_config(self, adapter_name: str) -> Dict:
        ...
        if os.path.exists(nested_config_path):
            ...
            # Convert nested learning config to adapter config format
            ...
            if os.path.exists(base_adapter_config_path):
                try:
                    with open(base_adapter_config_path, 'r') as f:
                        ...
                        # Use the base adapter's training data which should have Q&A format
                        training_data_path = base_config.get("data", training_data_path)
                        # If it's a directory, append train.jsonl
            ...
            adapter_config = {
                ...
                "learning_rate": nested_config.get("learning_rate"),
                "max_seq_length": nested_config.get("max_seq_length"),
                "lora_parameters": {
                    "rank": nested_config.get("lora_rank"),
        ...
        # Otherwise, load regular adapter config
    ...
    def generate_response(self, adapter_path: Optional[str], model_path: str, prompt: str) -> str:
        ...
        # For nested learning adapters, point to the best checkpoint
...
{{
  ...
  "explanation": "Brief explanation of scores"
    ...
    def evaluate_adapter(self, adapter_name: str, training_data_path: str, num_questions: int = 20, progress_callback=None, use_base_model: bool = False) -> Dict:
        ...
        # Determine adapter path (could be regular or nested learning)
        if use_base_model:
            adapter_path = None
        else:
            # Check if config has adapter_path (for nested learning adapters)
