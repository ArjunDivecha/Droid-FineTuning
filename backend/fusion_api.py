    ...
    if os.path.exists(nested_config_path):
        try:
            with open(nested_config_path, 'r') as f:
                ...
                # Convert nested learning config to session-like format
                return {
                    'config': {
                        'model_path': nested_config.get('base_model_path'),
                        'train_data_path': nested_config.get('train_data_path'),
                        'iterations': nested_config.get('num_steps'),
                        'adapter_name': adapter_name
                    },
                    'model_path': nested_config.get('base_model_path'),
                    'train_data_path': nested_config.get('train_data_path'),
                    'num_iters': nested_config.get('num_steps'),
                    'timestamp': None  # Would need to read from metrics or checkpoint timestamps
                }
        except Exception as e:
        ...
        "formats": [
            {
                "id": "mlx",
                "name": "MLX Format",
                "description": "Native MLX adapter format (safetensors)",
                "default": True
            },
            {
                "id": "gguf",
                "name": "GGUF",
                "description": "Convert to GGUF for llama.cpp, Ollama, LM Studio",
                "requires_conversion": True
            },
            {
                "id": "ollama",
                "name": "Ollama",
                "description": "Push to Ollama model library",
                "requires_ollama": True
            },
            {
                "id": "lm_studio",
                "name": "LM Studio", 
                "description": "Export to LM Studio models directory",
                "requires_path": True
            }
        ]
    }
...
@router.post("/export")
async def export_fused_adapter(request: Dict[str, Any]):
    """Export fused adapter to specified format."""
    try:
        adapter_name = request.get("adapter_name")
        export_format = request.get("format", "mlx")
        ...
        result = {"success": True, "format": export_format, "adapter_name": adapter_name}
        
        if export_format == "mlx":
            # Already in MLX format
            result["path"] = adapter_dir
            result["message"] = f"Adapter available in MLX format at {adapter_dir}"
            
        elif export_format == "gguf":
            result["message"] = "GGUF conversion requires llama.cpp tools (not implemented yet)"
            result["instructions"] = "Use llama.cpp convert script to convert the merged model to GGUF"
            ...
            try:
                ...
                result["message"] = "Ollama detected - use 'ollama create' to import the model"
                result["instructions"] = f"Run: ollama create {adapter_name} -f Modelfile"
            except (subprocess.CalledProcessError, FileNotFoundError):
                result["success"] = False
                result["message"] = "Ollama not found. Install from https://ollama.ai"
                
        elif export_format == "lm_studio":
            if not output_path:
                result["message"] = "Please provide LM Studio models directory path"
                result["default_path"] = os.path.expanduser("~/.cache/lm-studio/models")
            else:
                result["message"] = f"Copy adapter to: {output_path}"
                result["instructions"] = f"Copy {adapter_dir} to {output_path}/{adapter_name}"
